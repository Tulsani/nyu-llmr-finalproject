...
from sec_4.run_experiment import load_policy_into_vllm_instance, init_vllm, run_get_response_log_probs_util
from sec_4.sec4 import run_tokenize_prompt_and_output_util, run_masked_normalize_util
from grpo_run.dataloader_consistent import get_gsm_adversarial_dataloaders
from grpo_run.defaults import MODEL_NAME
from grpo_run.sec7 import run_compute_policy_gradient_loss_util, run_masked_mean_util, \
    run_compute_group_normalized_rewards_util
from drgrpo_grader import pit_reward_fn, extract_answer_pit, pit_reward_fn_consistent
...
"""
CONSISTENCY REWARD
This GRPO run is for reward based on correctness and consistency.
"""
...
def run_grpo_training(
    ...
    for step in range(n_grpo_steps):
        ...
        print("question groups", questions_batch["question_groups"])
        assert len(questions_batch["question_groups"]) == 1

        group = questions_batch["question_groups"][0]
        prompts = group["prompts"]
        gts = group["ground_truths"]
...
if __name__ == '__main__':
    ...
    # wandb things
    run_name = f"PIT-cons-GRPO-lt{loss_type}-ga{str(gradient_accumulation_steps)}-ngs{str(n_grpo_steps)}_lr{str(learning_rate)}_usestd{str(use_std_normalization)}_nt{normalize_type}"
    wandb.init(
        project=f"PIT-GRPO",
        name=run_name,
        config={
